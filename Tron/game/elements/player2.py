...
class Player2(Player):
    ...
    def __init__(self):
        """
        Creates Player 2 and sets up the lives, light cycle, and controls
        """
        super().__init__()
        #PLAYER NAME AND POSITION
        self._text = "Player 2"
        self._position = Point(constants.MAX_X-95, 10)

        #REMAINING LIVES POSITION
        self.lives.set_position(Point(constants.MAX_X-60, 32))
        #self.lives.set_offset(Point(self.lives.get_font_size(), 0))

        #LIGHT BIKE COLOR, STARTING POSITION, AND STARTING VELOCITY
        self.cycle.change_color(constants.ORANGE)
        self.cycle.set_position(Point(int(constants.MAX_X/2), constants.MAX_Y - self.cycle.get_font_size() - 10))
        self.cycle.set_velocity(Point(0, -constants.FONT_SIZE))

        #PLAYER SPECIFIC CONTROL KEYS
        self.cycle.set_player_keys(['i', 'j', 'k', 'l', 'rs'])